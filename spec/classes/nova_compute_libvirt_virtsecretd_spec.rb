...
eos
  ...
  shared_examples_for 'nova-compute-libvirt-virtsecretd' do
    ...
    context 'with default parameters' do
      ...
      it { is_expected.to contain_virtsecretd_config('log_level').with_ensure('absent')}
      it { is_expected.to contain_virtsecretd_config('log_outputs').with_ensure('absent')}
      it { is_expected.to contain_virtsecretd_config('log_filters').with_ensure('absent')}
      it { is_expected.to contain_virtsecretd_config('max_clients').with_ensure('absent')}
      it { is_expected.to contain_virtsecretd_config('admin_max_clients').with_ensure('absent')}
      it { is_expected.to contain_virtsecretd_config('ovs_timeout').with_ensure('absent')}
    end
    ...
    context 'with specified parameters' do
      ...
      it { is_expected.to contain_virtsecretd_config('log_outputs').with_value("\"#{params[:log_outputs]}\"")}
      it { is_expected.to contain_virtsecretd_config('log_filters').with_value("\"#{params[:log_filters]}\"")}
